...
- [ ] **V2 — Confirm the clean-clone build failure.** `git clone` to a fresh directory, `pnpm run setup && pnpm run build:host`. Expected: `tsc` fails on `@karaokenatin/shared`. *(I-1.3, gates T4)* — **S**
- [ ] **V3 — Confirm the auth bypass.** Connect a socket.io client sending `JOIN_ROOM {roomId: "", joinToken: ""}`. Expected: join succeeds. *(I-2.1, gates T5)* — **S**
...
- [ ] **T1 — Self-host the PeerJS broker.** *(I-2.5, O-0)* — **L**
...
- [ ] **T2 — Implement or remove the three phantom commands.** *(I-1.1)* — **S**
...
- [ ] **T3 — Fix the `playlist_import_collection` argument name.** *(I-1.2)* — **S**
...
- [ ] **T4 — Build `packages/shared` during setup.** *(I-1.3, O-12)* — **S**
...
- [ ] **T5 — Close or remove the join-token bypass.** *(I-2.1)* — **M** `[?]`
...
- [ ] **T6 — Sweep every `invoke` call site against its Rust signature.** *(I-1.2 note)* — **M**
...
- [ ] **T7 — Declare `SEARCH`/`SEARCH_RESULTS` in the protocol.** *(I-1.4)* — **S**
...
- [ ] **T8 — Add Linux bundle targets.** *(I-2.3)* — **S**
  `tauri.conf.json:33-36` lists only `nsis`/`msi`. Add `deb` and `appimage`.

- [ ] **T9 — Provide `.sh` equivalents of the three build scripts.** *(I-2.3)* — **M**
...
- [ ] **T10 — Remove the Windows `.exe` prerequisite from the top-level build.** *(I-2.3, I-3.1)* — **S**
...
- [ ] **T12 — Scope the Android network security config.** *(I-2.4)* — **S**
  `network_security_config.xml` permits cleartext to *all* domains and trusts user-installed CAs. Narrow to a `domain-config` covering private ranges; drop `<certificates src="user" />`.
...
- [ ] **T14 — Trim the Android manifest permissions.** *(I-2.6)* — **S**
...
- [ ] **T15 — Delete or document the dead Node signaling server.** *(I-3.1, O-4)* — **S** `[?]`
...
- [ ] **T16 — Delete or wire up the orphaned Next.js client.** *(I-3.2, O-4)* — **S** `[?]`
...
- [ ] **T18 — Vendor the remote-ui CDN dependencies.** *(I-4.1, O-3)* — **S**
...
- [ ] **T19 — Move guest-visibility filtering into Rust.** *(I-3.4)* — **S**
...
- [ ] **T20 — Render or delete `HelpDialog`.** *(I-3.3)* — **S**
  159 lines, never imported. Its two buttons call two of T2's phantom commands.

- [ ] **T21 — Purge committed build artifacts.** *(I-4.8, O-10)* — **S**
...
- [ ] **T22 — Build volume, mute, and seek UI.** *(I-3.3)* — **M**
...
- [ ] **T23 — Acquire a wake lock during playback.** *(I-4.4)* — **S**
...
- [ ] **T24 — Add P2P reconnection and liveness.** *(I-3.6)* — **L**
...
- [ ] **T25 — Expose remaining backend-only queue commands.** *(I-3.3)* — **S**
...
- [ ] **T26 — Decide what scoring should be.** *(FEATURES.md §8)* — **?** `[?]`
...
- [ ] **T33 — Add protocol round-trip tests.** *(I-4.9, O-9)* — **M**
...
- **Q1 — Are `apps/signaling-server` and `apps/web-client` coming back?** If there is a planned internet-hosted mode they are dormant infrastructure; if not they are ~1000 lines of misleading dead code. *Blocks T15, T16, T37; changes T17's scope.*
- **Q2 — Is the join token meant to be real security, or is open-LAN-join the intended UX?** Both are defensible; the current in-between is not. *Blocks T5.*
- **Q3 — Android TV (D-pad) or Android phone (touch) as the primary mobile target?** Decides whether virtualization must preserve focus semantics. *Materially changes T28.*
- **Q4 — Was deleting `.github/workflows/build-release.yml` deliberate?** *Blocks T11.*
- **Q5 — Should scoring become real, or be labelled as a gimmick?** *Blocks T26; a "real" answer changes T14.*
